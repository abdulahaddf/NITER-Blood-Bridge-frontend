    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Welcome to the NITER Blood Connect admin panel
        </p>
      </div>
            ...
            <div className="space-y-3">
              {bloodGroupStats.map((stat) => (
                <div key={stat.bloodGroup} className="flex items-center gap-4">
                  <span className={`blood-group-badge w-12 justify-center ${bloodGroupColors[stat.bloodGroup]}`}>
                    {BloodGroupLabels[stat.bloodGroup]}
                  </span>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm">{stat.eligibleCount} eligible</span>
                      <span className="text-sm text-muted-foreground">of {stat.count}</span>
                    </div>
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <div 
                        className={`h-full ${bloodGroupColors[stat.bloodGroup]}`}
                        style={{ width: `${(stat.eligibleCount / stat.count) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Deletion Requests', link: '/dashboard/deletions', icon: Trash2 },
              { label: 'View Analytics', link: '/dashboard/analytics', icon: TrendingUp },
              { label: 'Send Broadcast', link: '/dashboard/broadcast', icon: HeartPulse },
            ].map((action) => (
              <Button
                key={action.label}
                variant="outline"
